...
The Rack encoder is intentionally implemented as an endless relative control rather than a bounded Rack parameter. **Hold the left mouse button and drag vertically** to produce discrete hardware-style detents; mouse-wheel movement produces one detent per step. A stationary click/hold is resolved as the physical encoder push. One combined widget handles both gestures, so a separate centre switch cannot steal a drag and the visual indicator never reaches an artificial parameter boundary or jumps back to centre.

The Rack module uses a black functional front plate while retaining the simulator-defined geometry. The Rack-visible identity is **South Signal Lab Clock**. The top uses the canonical project-owned **CLOCK vector wordmark** rather than a text approximation; **SOUTH SIGNAL LAB** is centred more prominently at the bottom. Primary button functions **PLAY**, **TAP**, and **STOP** are white; secondary functions **PAUSE**, **SHIFT**, and **BACK** sit below them in grey without slash separators. Input jacks use only **IN 1** and **IN 2** on the panel, while their Rack tooltips retain the `SYNC` and `RST` function names. Outputs are numbered **1–8** below the jacks. The encoder remains intentionally unlabelled. Rack's NanoSVG loader does not render SVG `<text>` nodes, so the actual module draws lettering as a NanoVG overlay with Rack's bundled UI font; the generated SVG keeps matching text for standalone previews.

OUT 1–8 always expose the real instantaneous 0/+5 V CLOCK gate state. The panel's inactive LED apertures are deliberately dark, and Rack's built-in `setBrightnessSmooth()` provides a short bounded visual decay for brief gates instead of a manually latched activity timer.
