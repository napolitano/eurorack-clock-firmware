...
The editable end-user manual is maintained as [`clock-user-manual.odt`](clock-user-manual.odt). The browser-readable content baseline is [`../USER_GUIDE.md`](../USER_GUIDE.md); user-visible facts must remain synchronized between both forms. Its manual-specific license is documented in [`LICENSE.md`](LICENSE.md). The ODT currently represents the maintained working source; release preparation freezes a version-matched copy under `releases/<firmware-version>/`.
