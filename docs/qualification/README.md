...
`v1_qualification.json` is deliberately **not** a success report. A test stays `PENDING` or `BLOCKED` until representative hardware has actually been measured. `PASS` is valid only with at least one repository-relative evidence reference. `scripts/check_hil_qualification.py --require-pass-if-rc` becomes an all-PASS gate automatically for `1.0.0-rc.*` and `1.0.0`.
