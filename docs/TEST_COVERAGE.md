...
The prerelease test policy is repository-wide rather than limited to `clock_core`. `pio test -e native` exposes all 400 named Native cases, while `run_host_tests.py` remains the authoritative variant/sanitizer/coverage matrix.
