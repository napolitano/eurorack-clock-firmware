...
VirtualDrummer::VirtualDrummer(hal::OledDisplay& display, hal::ControlPanel& controls, hal::GateOutputDriver& gateOutputs)
    : display_(display), controls_(controls), gateOutputs_(gateOutputs) {}

void VirtualDrummer::run() {
    gateOutputs_.disableOutputStage(); gateOutputs_.setAllChannelsLow();
    resetSession(hal::SystemClock::milliseconds());
#ifdef CLOCK_HOST_TEST
    render();
    return;
#else
    gateOutputs_.enableOutputStage();
    std::uint32_t lastFrameAtMs = hal::SystemClock::milliseconds();
    while (!exitRequested_) {
        ...
        update(controls, nowMs);
        if (nowMs - lastFrameAtMs >= kFrameIntervalMs) { render(); lastFrameAtMs = nowMs; }
        hal::SystemClock::delayMilliseconds(1U);
    }
    stopOutputs(); gateOutputs_.disableOutputStage();
#endif
}

#ifdef CLOCK_SIMULATOR
void VirtualDrummer::beginForSimulator() {
    gateOutputs_.disableOutputStage(); gateOutputs_.setAllChannelsLow();
    const std::uint32_t nowMs = hal::SystemClock::milliseconds();
    resetSession(nowMs); gateOutputs_.enableOutputStage(); lastSimulatorFrameAtMs_ = nowMs; render();
}

bool VirtualDrummer::serviceForSimulator(const std::uint32_t nowMs) {
    const hal::ControlSample controls = controls_.sample(nowMs);
    if (controls.encoderButton.edge == hal::ButtonEdge::Pressed) encoderPressedAtMs_ = nowMs;
    if (!controls.encoderButton.pressed) encoderPressedAtMs_ = 0U;
    else if (encoderPressedAtMs_ != 0U && nowMs - encoderPressedAtMs_ >= kExitLongPressMs) exitRequested_ = true;
    update(controls, nowMs);
    if (nowMs - lastSimulatorFrameAtMs_ >= kFrameIntervalMs) { render(); lastSimulatorFrameAtMs_ = nowMs; }
    if (exitRequested_) { stopOutputs(); gateOutputs_.disableOutputStage(); }
    return !exitRequested_;
}
#endif

void VirtualDrummer::resetSession(const std::uint32_t nowMs) {
...
std::uint32_t VirtualDrummer::stepDurationMs() const {
    return 15000U / static_cast<std::uint32_t>(bpm_); // quarter note / 4 = 16th note
}

void VirtualDrummer::stopOutputs() {
    gateOutputs_.setAllChannelsLow(); gatesHigh_ = false;
}

void VirtualDrummer::triggerStep(const std::uint32_t nowMs) {
    stopOutputs();
...
void VirtualDrummer::update(const hal::ControlSample& controls, const std::uint32_t nowMs) {
